...
const logger = log4js.getLogger('controllers - getInvoiceHistory');
...
invoice.getInvoiceHistory = async (req, res, next) => {
  logger.debug('entering >>> getInvoiceHistory()');
  ...
  try {
    ...
    const invokeResponse = await contract.evaluateTransaction('getInvoiceHistory', invoiceNo);
    ...
    logger.debug('Error evaluating transaction getInvoiceHistory...');
  ...
  logger.debug('exiting <<< getInvoiceHistory()');
