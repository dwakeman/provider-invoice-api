...
const logger = log4js.getLogger('controllers - invoice');
...
invoice.getInvoices = async (req, res, next) => {
  logger.debug('entering >>> getInvoices()');
  ...
  try {
    ...
    const invokeResponse = await contract.evaluateTransaction('getInvoices', 'ALL');
...
//      result: invokeResponse.toString(),
    ...
    logger.debug('Error evaluating transaction getInvoices...');
  ...
  logger.debug('exiting <<< getInvoices()');
