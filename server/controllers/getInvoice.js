...
const logger = log4js.getLogger('controllers - getInvoice');
...
invoice.getInvoice = async (req, res, next) => {
  logger.debug('entering >>> getInvoice()');
  ...
  try {
    ...
    const invokeResponse = await contract.evaluateTransaction('getInvoice', invoiceNo);
    ...
    logger.debug('Error evaluating transaction getInvoice...');
  ...
  logger.debug('exiting <<< getInvoice()');
